...
class UserAddressListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return self.model.objects.filter(status=UserAddress.STATUS_NORMAL, user=self.request.user)
...
class UserAddressCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        obj.status = UserAddress.STATUS_NORMAL
        obj.save()
        return HttpResponseRedirect(reverse('account:user_addresses'))
...
class UserAddressDeleteView(LoginRequiredMixin, ObjectPermMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        obj = self.get_object()
        obj.status = UserAddress.STATUS_DELETE
        obj.save()
        return HttpResponseRedirect(reverse('account:user_addresses'))
