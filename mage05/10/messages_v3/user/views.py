...
from . import models
from .forms import LoginForm, CreateUserForm, ModifyUserForm, PasswordForm, PasswordForm2
...
def modify_password(request):
    ...
    if request.method == 'GET':
        try:
            _id = request.GET.get('id', 0)
            user = models.User2.objects.get(pk=_id)
            form = PasswordForm2(request.GET, instance=user)
        except ObjectDoesNotExist as e:
            form = PasswordForm()
    else:
        form = PasswordForm2(request.POST)
        if form.is_valid():
            user = models.User2.objects.get(pk=form.cleaned_data['id'])
